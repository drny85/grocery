...
exports.addItem = asyncHandler(async (req, res, next) => {
        ...
        req.user = {"_id":"5dae0b1b73640829ac2f5ab7","name":"robert"};
        ...
        const checkItem = await Item.findOne({name: tempItem.name});
        if (checkItem) {
        ...
        fileName = `${req.user._id}-${Date.now()}${path.parse(file.name).ext}`;
...
exports.updateItem = asyncHandler(async (req, res, next) => {
        ...
        const item = await Item.findByIdAndUpdate(itemId, req.body, {new: true, runValidators: true})
